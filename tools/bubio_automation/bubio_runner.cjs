...
Common options:
  ...
  --chrome-profile-dir   Chrome profile directory name, e.g. "Profile 6" or "Default"
...
Examples:
  ...
  bubio_runner.sh bootstrap-auth-from-profile --chrome-profile-dir "Profile 6"
...
async function runBootstrapAuthFromProfile(options) {
  const stateFile = path.resolve(expandHome(options["state-file"] || DEFAULT_STATE_FILE));
  const profileDirName = options["chrome-profile-dir"] || "Profile 6";
